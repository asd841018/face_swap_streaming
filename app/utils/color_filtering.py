...
def cold_white_ycrcb(bgr, strength=0.35, brighten=6):
    """
    strength: 0~1 冷白力度
...
def beauty_pipeline(frame, cold=0.35, brighten=6, smooth=0.55):
    # 1) 冷白（順便拿 ycrcb 給 mask 用）
    out, ycrcb = cold_white_ycrcb(frame, strength=cold, brighten=brighten)
